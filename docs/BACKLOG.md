...
| R-11 | P1 | open | retry `scope=shards` resets shards to pending but enqueues an embed job the embedder ignores → failed shards never re-parsed; doc flips ready/partial with a stranded pending shard | no ParseJob enqueued; embedder selects `state=="done"` only | this fix |
| R-12 | P0 | open | second document's chunks overwrite first document's Qdrant points; citations can attribute the wrong book | point id derived from chunk_hash alone; UNIQUE(doc_id, chunk_hash) is composite; hydrate keys rows by chunk_hash only | this fix |
| R-13 | P3 | open | all windows of a long section cite the section's full page span | `_emit` indexes the page map with the section's line bounds, not the window's | this fix |
| R-14 | P2 | open | API key scoped to collection A returns < top_k when B-chunks fill slots | key-level collection scope post-filters after Qdrant top_k truncation (MCP path pushes down) | this fix |
| R-15 | P2 | open | dedupe key and stored hash are client-controlled | commit stores `body.content_sha256` without reading the object (PRD §6.1 says API computes it) | this fix |
| R-16 | P3 | open | TEI embed path batches at a budget pinned to ollama's 2048 ceiling; tokenizer reloaded per job | `ctx_budget = 1900` + `from_pretrained("BAAI/bge-m3")` hardcoded in handle_embed | this fix |
| R-17 | P2 | open | one ~12MB blocking upsert per book re-creates the 2026-09-11 timeout-loop shape | `upsert_chunks` sends all points in a single `wait=True` request | this fix |
| R-18 | P3 | open | `chunks.embedded_at` never stamped; re-embed tooling cannot identify stale vectors | embedder insert omits the column | this fix |
| R-19 | P3 | open | eval JSON reports `hit_at_{top_k}` carrying the hit@8 value; undercounts for top_k > 8 | run.py conflates judge's fixed hit@8 with run top_k | this fix |
| R-20 | P3 | open | `/pipeline` shows a dead Redis as empty lanes while `/queues` propagates | lanes loop `except Exception: pass` | this fix |
| R-21 | P2 | open | non-PDF / over-cap PDFs are discovered inside a parser instead of rejected at commit | no magic-byte / page-count validation (PRD §11) | this fix |
